...
from textrevised import tokenize, bert_embed, BERT_MODEL_DIM
...
class VGGLoss(nn.Module):
    def __init__(self):
        ...
        self.vgg_layers = nn.Sequential(*vgg)
    ...
    def forward(self, x, y):
        ...
        y_flat = y.view(b * f, c, h, w)
        x_vgg = self.vgg_layers(x_flat)
        y_vgg = self.vgg_layers(y_flat)
...
class GaussianDiffusion(nn.Module):
    ...
    @torch.inference_mode()
    def sample(self, cond=None, cond_scale=1., batch_size=4):
    ...
    def p_losses(self, x_start, t, cond=None, noise=None, **kwargs):
        ...
        if self.loss_type == 'l1':
            ...
            loss_reconstruction = F.mse_loss(noise, x_recon)
        else:
...
class Trainer(object):
    def __init__(
        ...
        *,
        ...
        num_sample_rows=4,
        max_grad_norm=None,
        loss_log_file='./results/loss_log.txt'
    ):
        super().__init__()
        ...
        self.ds = Dataset(folder, self.image_size, channels=diffusion_model.channels, num_frames=num_frames)
        ...
        # Create the checkpoints folder inside the results folder
        self.checkpoints_folder = self.results_folder / 'checkpoints'
        self.checkpoints_folder.mkdir(exist_ok=True, parents=True)
        self.vgg_loss = VGGLoss().to(device)  # Ensure it's on the correct device
    ...
    def save(self, milestone):
        # Save model checkpoint inside the checkpoints folder
        checkpoint_data = {
            'step': self.step,
        ...
        torch.save(checkpoint_data, str(self.checkpoints_folder / f'model-{milestone}.pt'))
    ...
    def extract_info_from_path(self, path):
        ...
        """
        return Path(path).stem  # Get the file name without the extension
    ...
    def train(
        ...
        with open(self.loss_log_file, 'w') as loss_log:
            loss_log.write("Step, Loss, VGG Loss, L1 Loss\n")  # Corrected header

            # Initialize tqdm progress bar
            pbar = tqdm(total=self.train_num_steps, desc="Training Progress")
            ...
            while self.step < self.train_num_steps:
                for i in range(self.gradient_accumulate_every):
                    ...
                    with autocast(enabled=self.amp):
                        loss, perceptual_loss, loss_reconstruction = self.model(
                            data,
                    ...
                    print(
                        f'Step {self.step}: Loss = {loss.item()}, VGG Loss = {perceptual_loss.item()}, L1 Loss = {loss_reconstruction.item()}')

                # Corrected logging
                log = {
                    'loss': loss.item(),
                    'VGG Loss': perceptual_loss.item(),
                    'L1 Loss': loss_reconstruction.item()
                }
                loss_log.write(f"{self.step}, {loss.item()}, {perceptual_loss.item()}, {loss_reconstruction.item()}\n")
                ...
                if self.step % self.update_ema_every == 0:
                    ...
                    if self.step != 0 and self.step % self.save_and_sample_every == 0:
                        ...
                        # Sample one GIF at a time
                        video_tensor = self.ema_model.sample(batch_size=1).squeeze(0)

                        # Get the original file name from the dataset path
                        original_path = self.ds.paths[self.step % len(self.ds.paths)]
                        original_filename = self.extract_info_from_path(original_path)

                        # Construct the directory using the epoch number
                        save_dir = self.results_folder / f"epoch_{milestone}"
                        save_dir.mkdir(parents=True, exist_ok=True)

                        # Save the GIF with the original filename
                        gif_name = f"{original_filename}.gif"
                        video_path = save_dir / gif_name
                        video_tensor_to_gif(video_tensor, str(video_path))

                        # Log the saved GIF path
                        log['sample'] = str(video_path)

                        torch.cuda.empty_cache()

                        # Save model checkpoint after every save_and_sample cycle
                        self.save(milestone)

                log_fn(log)
                self.step += 1  # Moved outside the conditional block
...
def test_video_saving(save_folder):
    # Ensure the folder exists
    save_folder = Path(save_folder)
    save_folder.mkdir(exist_ok=True, parents=True)

    # Generate a random tensor resembling a video
    random_video_tensor = torch.randn(3, 8, 64, 64)  # (channels, frames, height, width)

    # Define the path to save the GIF
    video_path = save_folder / 'test_video.gif'

    # Save the tensor as a GIF
    video_tensor_to_gif(random_video_tensor, str(video_path))

    print(f'Saved test video to {video_path}')


def test_model_sampling(model, save_folder, num_samples=4):
    # S'assurer que le dossier de sauvegarde existe
    save_folder = Path(save_folder)
    save_folder.mkdir(exist_ok=True, parents=True)

    # Générer des vidéos à partir du modèle
    with torch.no_grad():
        model.eval()
        videos = model.sample(batch_size=num_samples)

    # Sauvegarder chaque vidéo comme GIF
    for i, video in enumerate(videos):
        video_path = save_folder / f'sample_video_{i + 1}.gif'
        video_tensor_to_gif(video, str(video_path))
        print(f"Saved sample video to {video_path}")


if __name__ == "__main__":
    # Initialize model and trainer parameters
    denoise_fn = Unet3D(dim=64).to(device)
    ...
    diffusion_model = GaussianDiffusion(
        ...
        channels=3,
        timesteps=1000,
        use_dynamic_thres=True,  # Enable dynamic thresholding
        dynamic_thres_percentile=0.95,
        # Try different percentile values (0.8, 0.9, 0.95)
    ).to(device)

    folder = r'C:\Users\DELL\Desktop\video-diffusion-pytorch\video_diffusion_pytorch\gifss'
    ...
    trainer = Trainer(
        ...
        ema_decay=0.995,
        save_and_sample_every=10  # Modifiez cette valeur pour sauvegarder et échantillonner plus fréquemment
    )
    ...
    save_folder = r'C:\Users\DELL\Desktop\video-diffusion-pytorch\video_diffusion_pytorch\testgifs'

    # Test de génération de vidéos
    test_model_sampling(diffusion_model, save_folder, num_samples=4)