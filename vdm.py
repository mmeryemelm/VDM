        ...
        with open(self.loss_log_file, 'w') as loss_log:
            ...
            while self.step < self.train_num_steps:
                for i in range(self.gradient_accumulate_every):
                    ...
                    print(f'Step {self.step}: Loss = {loss.item()}, VGG Loss = {perceptual_loss.item()}, L1 Loss = {loss_reconstruction.item()}')
                log = {'loss': loss.item()}
                log = {'VGG Loss': {perceptual_loss.item()}}
                log = {'L1 Loss: {loss_reconstruction.item()'}
